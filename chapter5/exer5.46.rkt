      ...
      (if (empty? the-ready-queue)
        ...
        (dequeue the-ready-queue
          (lambda (first-ready-thread other-ready-threads)
            ...
            (if (procedure? first-ready-thread)
                (begin
                  (set! the-time-remaining the-max-time-slice)
                  (eopl:printf "max time ~s~n" the-time-remaining)
                  (first-ready-thread))
                (begin
                 
                  (set! the-time-remaining (car first-ready-thread))
                  (eopl:printf "current time ~s~n" the-time-remaining)
  ...
  (define value-of/k                    
    (lambda (exp env cont)
      ...
      (cases expression exp
        ...
        (yield-exp ()
          (place-on-ready-queue!
           (let ((t the-time-remaining))
            (list t (lambda () (apply-cont cont (num-val 99))))))
          (run-next-thread))
