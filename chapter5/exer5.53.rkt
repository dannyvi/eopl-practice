...
(define trace-thread (make-parameter #t))
...
;;;;;;;;;;;;;;;; continuations ;;;;;;;;;;;;;;;;
  ...
  (define-datatype continuation continuation?
    ...
    (end-main-thread-cont (num expval?))           
    (end-subthread-cont (num expval?))
...
(define thread-id 'uninitialized)
...
(define new-thread-id
  (lambda ()
    (let ((current-id thread-id))
      (set! thread-id (num-val (+ 1 (expval->num thread-id))))
      current-id)))
  ...
  (define value-of/k                    
    (lambda (exp env cont id)
      ...
      (cases expression exp
        ...
        (const-exp (num) (apply-cont cont (num-val num) id))

        (const-list-exp (nums)
          (apply-cont cont
            (list-val (map num-val nums)) id))

        (var-exp (var) (apply-cont cont (deref (apply-env env var)) id))
  
        (diff-exp (exp1 exp2)
          (value-of/k exp1 env
            (diff1-cont exp2 env cont) id))

        (if-exp (exp1 exp2 exp3)
          (value-of/k exp1 env
            (if-test-cont exp2 exp3 env cont) id))

        (proc-exp (var body)
          (apply-cont cont
            (proc-val
              (procedure var body env)) id))

        (call-exp (rator rand)
          (value-of/k rator env
            (rator-cont rand env cont) id))
        ...
        (set-exp (var exp)
          (value-of/k exp env
            (set-rhs-cont (apply-env env var) cont) id))
        ...
        (yield-exp ()
          (place-on-ready-queue!
            (lambda () (apply-cont cont (num-val 99) id)))
          (run-next-thread))

        (mutex-exp ()
          (apply-cont cont (mutex-val (new-mutex)) id))  
      ...
      (if (time-expired?)
        ...
        (begin
          ...
          (cases continuation cont
            ...
            (end-main-thread-cont (num)
              (eopl:printf "main thread ~a terminated: ~s~%" id num)
              (set-final-answer! val)
              (run-next-thread))

            (end-subthread-cont (num)
              (eopl:printf "returned thread ~s: ~s~%" id num)
              (run-next-thread)
              )

            (diff1-cont (exp2 saved-env saved-cont)
              (value-of/k exp2 saved-env (diff2-cont val saved-cont) id))
            ...
            (if-test-cont (exp2 exp3 env cont)
              (if (expval->bool val)
                (value-of/k exp2 env cont id)
                (value-of/k exp3 env cont id)))
            (rator-cont (rand saved-env saved-cont)
              (value-of/k rand saved-env
                (rand-cont val saved-cont) id))
            (rand-cont (val1 saved-cont)
              (let ((proc (expval->proc val1)))
                (apply-procedure proc val saved-cont id)))
            (set-rhs-cont (loc cont)
              (begin
                (setref! loc val)
                (apply-cont cont (num-val 26) id)))

            (spawn-cont (saved-cont)
              (let ((proc1 (expval->proc val)))
                (place-on-ready-queue!
                  (lambda ()
                    (let ((num (new-thread-id)))
                      (when (trace-thread)
                        (eopl:printf 
                         "thread: allocating thread ~n~s~n with initial id ~n~s~%"
                         proc1 num))                     
                    (apply-procedure proc1
                      num
                      (end-subthread-cont num) num))))
              (apply-cont saved-cont (num-val 73) id)))

            (wait-cont (saved-cont)
              (wait-for-mutex
                (expval->mutex val)
                (lambda () (apply-cont saved-cont (num-val 52) id))))

            (signal-cont (saved-cont)
              (signal-mutex
                (expval->mutex val)
                (lambda () (apply-cont saved-cont (num-val 53) id))))

            (unop-arg-cont (unop1 cont)
              (apply-unop unop1 val cont id))

            )))))

  (define apply-procedure
    (lambda (proc1 arg cont id)
      (cases proc proc1
  ...
  (define apply-unop
    (lambda (unop1 arg cont id)
      (cases unop unop1

        (zero?-unop ()
          (apply-cont cont
            (bool-val
              (zero? (expval->num arg))) id))

        (car-unop ()
          (let ((lst (expval->list arg)))
            (apply-cont cont (car lst) id)))
        (cdr-unop ()
          (let ((lst (expval->list arg)))
            (apply-cont cont (list-val (cdr lst)) id)))

        (null?-unop ()
          (apply-cont cont 
            (bool-val (null? (expval->list arg))) id))

        (print-unop ()
          (begin
            (eopl:printf "~a~%" (expval->num arg))
            (apply-cont cont (num-val 1) id)))
