...
(define-datatype a-thread a-thread?
  (cont-thread (apply-cont procedure?)
          (cont continuation?)
          (val expval?))
  (proc-thread (apply-procedure procedure?)
               (proc1 proc?)
               (val expval?)
               (cont continuation?)))
        ...
        (dequeue the-ready-queue
          (lambda (first-ready-thread other-ready-threads)
            ...
            (cases a-thread first-ready-thread
              (cont-thread (apply-cont cont val) (apply-cont cont val))
              (proc-thread (apply-proc proc val cont) (apply-proc proc val cont)))
            ;(first-ready-thread)
            )))))
  ...
  (define value-of/k                    
    (lambda (exp env cont)
      ...
      (cases expression exp
        ...
        (yield-exp ()
          (place-on-ready-queue!
            (cont-thread apply-cont cont (num-val 99)))
          (run-next-thread))
  ...
  (define apply-cont                    
    (lambda (cont val)
      (if (time-expired?)
        (begin
          (place-on-ready-queue!
            (cont-thread apply-cont cont val))
          (run-next-thread))
        (begin
