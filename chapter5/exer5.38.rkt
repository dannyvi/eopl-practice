...
(define value-of/k
  (lambda (exp env cont excpt)
    (cases expression exp
      ...
      (div-exp (exp1 exp2)
                (value-of/k exp1 env (div1-cont exp2 env cont) excpt))
      (unop-exp (unop exp1)
  ...
  (lambda (cont excpt val)
    (cases continuation cont
      ...
      (div1-cont (exp2 saved-env saved-cont)
          (value-of/k exp2 saved-env (div2-cont val saved-cont) excpt))
      (div2-cont (val1 saved-cont)
                  (let ((n1 (expval->num val1))
                        (n2 (expval->num val)))
                    (if (zero? n2)
                        (apply-handler 0 (division-except))
                        (apply-cont saved-cont excpt
                                    (num-val (/ n1 n2))))))
    ...
    (cases exception excpt
           ...
           (base-except (var exp1 saved-env saved-cont saved-excpt)
                        (value-of/k exp1
                                    ...
                                    saved-excpt))
           (division-except ()
              (eopl:error 'division-except "division by zero.")))))
