...
(define the-grammar
  '((program (expression) a-program)
    ...
    (expression ("goback" expression) goback-exp)
...
(define-datatype exception exception?
  ...
  (try-except
   ...
   (cont continuation?)
   (excpt exception?))
  (raise-except (cont continuation?)
                (excpt exception?)))
...
(define apply-handler
  (lambda (val back-cont excpt)
    (cases exception excpt
           (end-except () (eopl:error 'apply-handler "uncaught exception"))
           (try-except (var handler-exp saved-env saved-cont saved-excpt)
                        (value-of/k handler-exp
                                    (extend-env var val saved-env)
                                    ;(end-cont)
                                    saved-cont
                                    ;back-cont
                                    (raise-except back-cont saved-excpt)))
           (raise-except (saved-cont saved-excpt)
                         (apply-cont saved-cont saved-excpt val)))))
...
(define apply-cont
  (lambda (cont excpt val)
    (cases continuation cont
      ...
      (raise1-cont (back-cont )
        ;; we put the short argument first to make the trace more readable.
                   (apply-handler val back-cont excpt))
      (goback-cont (saved-cont )
                   (apply-handler val saved-cont excpt ))
                   ;(apply-rollback val saved-cont excpt ))
      )))
  ...
  (lambda (exp env cont excpt)
    (cases expression exp
      ...
      (try-exp (exp1 var handler-exp)
        (value-of/k exp1 env cont (try-except var handler-exp env cont excpt)))
      (raise-exp (exp1)
                 (value-of/k exp1 env (raise1-cont cont) excpt)
                 )
      (goback-exp (exp1)
                 (value-of/k exp1 env (goback-cont cont) excpt))
      )))
