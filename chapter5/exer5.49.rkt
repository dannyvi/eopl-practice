...
(require (only-in racket first second third fourth))
...
;;
;;    (end-main-thread-cont (cont list?))           
;;    (end-subthread-cont  (cont list?))
;;
...
(define (continuation? cont) (list? cont))

(define (end-main-thread-cont) (cons (list 'end-main-thread-cont) '()))

(define (end-subthread-cont) (cons (list 'end-subthread-cont) '()))

(define (diff1-cont exp2 env cont) (cons (list 'diff1-cont exp2 env) cont))

(define (diff2-cont val1 cont) (cons (list 'diff2-cont val1) cont))

(define (if-test-cont exp2 exp3 env cont)
  (cons (list 'if-test-cont exp2 exp3 env) cont))

(define (rator-cont rand env cont) (cons (list 'rator-cont rand env) cont))

(define (rand-cont val1 cont) (cons (list 'rand-cont val1) cont))

(define (set-rhs-cont loc cont) (cons (list 'set-rhs-cont loc) cont))

(define (spawn-cont cont) (cons (list 'spawn-cont) cont))

(define (wait-cont cont) (cons (list 'wait-cont) cont))

(define (signal-cont cont) (cons (list 'signal-cont) cont))

(define (unop-arg-cont unop1 cont) (cons (list 'unop-arg-cont unop1) cont))
    ...
    (lambda (cont val)
      (if (time-expired?)
        ...
        (begin
          ...
          (decrement-timer!)
          (let* ((sign (caar cont))
                 (sign? (lambda (op) (eqv? sign op)))
                 (args (cdar cont))
                 (saved-cont (cdr cont)))
            (cond
              [(sign? 'end-main-thread-cont)
               (set-final-answer! val)
               (run-next-thread)]
              [(sign? 'end-subthread-cont)
               (run-next-thread)]
              [(sign? 'diff1-cont)
               (value-of/k (first args) (second args)
                           (diff2-cont val saved-cont))]
              [(sign? 'diff2-cont)
               (let ((n1 (expval->num (car args)))
                     (n2 (expval->num val)))
                 (apply-cont saved-cont (num-val (- n1 n2))))]
              [(sign? 'if-test-cont)
               (if (expval->bool val)
                   (value-of/k (first args) (third args) saved-cont)
                   (value-of/k (second args) (third args) saved-cont))]
              [(sign? 'rator-cont)
               (value-of/k (first args) (second args) (rand-cont val saved-cont))]
              [(sign? 'rand-cont)
               (let ((proc (expval->proc (first args))))
                 (apply-procedure proc val saved-cont))]
              [(sign? 'set-rhs-cont)
               (begin
                 (setref! (first args) val)
              ...
              [(sign? 'unop-arg-cont)
               (apply-unop (first args) val saved-cont)]
              ))
