  ...
  (lambda (exp env cont)
    (cases expression exp
      ...
      (letcc-exp (var body)
                 (let* ((sym (gensym))
                        (sym2 (gensym))
                        (new-env (extend-env sym cont env))
                        (nproc (proc-val
                                (procedure
                                 sym2
                                 (throw-exp (var-exp sym2) (var-exp sym))
                                 new-env))))
                   (value-of/k body (extend-env var nproc new-env) cont)
                 ))
      (throw-exp (exp1 exp2)
