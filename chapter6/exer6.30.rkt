...
(define fff-simple-exp
  (lambda (exp)
    (cases simple-expression exp
      (cps-const-exp (num) (number->string  num))
      (cps-var-exp (var) (symbol->string var))
      (cps-zero?-exp (exp1)
        (string-join (list "(zero?" (fff-simple-exp exp1) ")")))
      (cps-diff-exp (exp1 exp2)
        (string-join (list "(-" (fff-simple-exp exp1) 
                           (fff-simple-exp exp2) ")")))
      (cps-sum-exp (exps)
        (string-join (map fff-simple-exp exps)
          #:before-first "(+" #:after-last ")"))
      (cps-proc-exp (vars body)
        (let ((var-str (string-join (map symbol->string vars)
          #:before-first "(" #:after-last ")"))
              (body-str (fff-string body)))
          (string-join (list "λ" var-str "\n" body-str )
                       #:before-first "(" #:after-last ")")))
      )))
...
(define fff-string
  (lambda (expression)
    (cases tfexp expression
      (simple-exp->exp (exp) (fff-simple-exp exp))
      (cps-let-exp (var exp1 body)
        (string-join (list "(let" "(("(symbol->string var)
                           (fff-simple-exp exp1) "))"
                           (fff-string body) ")")))
      (cps-letrec-exp (p-vars b-varss p-exps letrec-body)
        (let ((proc-str (map (lambda (p-name b-vars p-exp)
          (string-join (list "(" (symbol->string p-name)
                      "(λ" "(" (fff-vars b-vars) ")~%"
                      (fff-string p-exp) "))"))) p-vars b-varss p-exps)))
          (string-join
           (list "(letrec"
                 (string-join proc-str) (fff-string letrec-body) ")"))))
      (cps-if-exp (exp1 exp2 exp3)
        (string-join (list "(if" (fff-simple-exp exp1)
                            (fff-string exp2)
                            (fff-string exp3)")")))
      (cps-call-exp (exp exps)
        (string-join (map fff-simple-exp (append (list exp) exps))
                     #:before-first "(" #:after-last ")")))))

(define fff-print
  (lambda (cpsed-pgm)
    (cases cps-out-program cpsed-pgm
           (cps-a-program (exp1) (eopl:printf "~a~%~%" (fff-string exp1))))))
  ...
  (define cps-of-program
    (lambda (pgm)
      (cases program pgm
        (a-program (exp1)
          (cps-a-program
            (cps-of-exp/ctx exp1 (lambda (x) (simple-exp->exp x))))))))
...
(define cps-of-exp/ctx
      ...
      (cases expression exp
        ...
        (zero?-exp (exp1)
          (cps-zero?-exp
            (cps-of-simple-exp exp1)))
        (proc-exp (ids exp)
          (cps-proc-exp (append ids (list 'k%))
            (cps-of-exp exp (cps-var-exp 'k%))))
        (sum-exp (exps)
          (cps-sum-exp
  ...
  (define cps-of-zero?-exp
    (lambda (exp1 k-exp)
      (cps-of-exp/ctx exp1
        (lambda (simp1)
          (make-send-to-cont k-exp (cps-zero?-exp simp1))))))
      ;(cps-of-exps (list exp1)
      ;  (lambda (new-rands)
      ;    (make-send-to-cont
      ;      k-exp
  ...
  (define cps-of-if-exp
    (lambda (exp1 exp2 exp3 k-exp)
      (cps-of-exp/ctx exp1
        (lambda (simp1)
          (cps-if-exp simp1 (cps-of-exp exp2 k-exp)
  ...
  (define cps-of-letrec-exp
    (lambda (proc-names idss proc-bodies body k-exp)
      (cps-letrec-exp
        proc-names
        (map
          (lambda (ids) (append ids (list 'k%)))
          idss)
        (map
          (lambda (exp) (cps-of-exp exp (cps-var-exp 'k%)))
          proc-bodies)
        (cps-of-exp body k-exp))))
    ...
    (lambda (rator rands k-exp)
      (cps-of-exp/ctx rator
        (lambda (simp1)
          (cps-of-exps rands
            (lambda (new-rands)
              (cps-call-exp simp1 (append new-rands (list k-exp)))))))))
...
(define run
  (lambda (string)
    (let ((cpsed-pgm
      ...
      (when (instrument-cps) (fff-print cpsed-pgm))
