...
;;;;;;;;;;;;;;;; initial environment ;;;;;;;;;;;;;;;;
 ...
 (define cps-out-grammar
   ...
   '((cps-out-program (tfexp) cps-a-program)
      ...
      (atom-expression (number) cps-const-exp)
      (atom-expression (identifier) cps-var-exp)
      (simple-expression (atom-expression) cps-atom-exp)
      (simple-expression
       ("-" "(" atom-expression "," atom-expression ")") cps-diff-exp)
      (simple-expression ("zero?" "(" atom-expression ")") cps-zero?-exp)
      (simple-expression
...
(define value-of-atom-exp
  (lambda (exp env)
    (cases atom-expression exp
  ...
  (define value-of-simple-exp
    (lambda (exp env)
      (cases simple-expression exp
        (cps-atom-exp (atom) (value-of-atom-exp atom env))
        (cps-diff-exp (atom1 atom2)
          (let ((val1 (expval->num (value-of-atom-exp atom1 env)))
                (val2 (expval->num (value-of-atom-exp atom2 env))))
            (num-val (- val1 val2))))
        (cps-zero?-exp (atom1)
          (bool-val (zero? (expval->num (value-of-atom-exp atom1 env)))))
        (cps-sum-exp (exps)
...
(define value-of-number-or-identifier
  (lambda (var env)
    (if (number? var) (num-val var) (apply-env env var))))
