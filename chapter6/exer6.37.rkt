...
;;                  (cps-of-rest
;;                    (list-set exps pos (cps-var-exp var)))))))))))
    ...
    (let cps-of-rest ([exps exps] [acc '()])
      (cond [(null? exps) (builder (reverse acc))]
            ...
            [else (let ([var (fresh-identifier 'var)])
              (cps-of-exp (car exps) (cps-proc-exp (list var)
               (cps-of-rest (cdr exps) (cons (cps-var-exp var) acc)))))]))))
...
(define cps-of-let-exp
  (lambda (id rhs body k-exp)
    ...
    (cps-of-exps (list rhs)
      (lambda (new-rands)
        (cps-newrefk-exp (car new-rands)
          (cps-proc-exp (list id) (cps-of-exp body k-exp)))))))
    ...
    (lambda (exp env)
      ...
      (cases simple-expression exp
        ...
        (cps-var-exp (var) (apply-env env var))
                                 ;(deref (expval->ref (apply-env env var))))
