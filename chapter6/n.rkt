...
;; Exercise 6.37 [★★★] Add implicit references (section 4.3) to CPS-IN. Use the same version of CPS-OUT, with
;; explicit references, and make sure your translator inserts allocation and dereference where necessary. As a hint,
;; recall that in the presence of implicit references, a var-exp is no longer simple, since it reads from the store.
...
(define cps-of-let-exp
  (lambda (id rhs body k-exp)
    (cps-of-exps (list rhs)
                 (lambda (new-rands)
                   (cps-newrefk-exp (car new-rands) (cps-proc-exp (list id) (cps-of-exp body k-exp)))))))

(define cps-of-letrec-exp
  (lambda (proc-names idss proc-bodies body k-exp)
    (let loop1 ([proc-names1 proc-names])
      (if (null? proc-names1)
          (let loop2 ([proc-names2 proc-names]
                      [idss2 idss]
                      [proc-bodies2 proc-bodies])
            (if (null? proc-names2)
                (cps-of-exp body k-exp)
                (cps-setrefk-exp (cps-var-exp (car proc-names2))
                                 (cps-proc-exp (append (car idss2) (list 'k%00))
                                               (cps-of-exp (car proc-bodies2) (cps-var-exp 'k%00)))
...
(define cps-of-exp
  (lambda (exp k-exp)
    (cases expression exp
      ...
      [var-exp (var) (cps-derefk-exp (cps-var-exp var) k-exp)]
  ...
  (lambda (exps builder)
    (let cps-of-rest ([exps exps]
      ...
      (cond [(null? exps) (builder (reverse acc))]
            [(inp-exp-simple? (car exps)) (cps-of-rest (cdr exps)
