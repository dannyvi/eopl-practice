  ...
  (define value-of-simple-exp
    (lambda ()
      (cases simple-expression exp
        (cps-const-exp (num) (set! pc #f) (set! val (num-val num)))
        (cps-var-exp (var) (set! pc #f) (set! val (apply-env env var)))
        (cps-diff-exp (exp1 exp2)
                (let ((val1 (begin (set! exp exp1)
                                   (set! pc value-of-simple-exp)
                                   (expval->num (trampoline!))))
                      (val2 (begin (set! exp exp2)
                                   (set! pc value-of-simple-exp)
                                   (expval->num (trampoline!)))))
                  (set! pc #f)
                  (set! val (num-val (- val1 val2)))))
        (cps-zero?-exp (exp1)
          (set! exp exp1)
          (set! pc value-of-simple-exp)
          (set! val (bool-val (zero? (expval->num (trampoline!)))))
          (set! pc #f))
        (cps-sum-exp (exps)
          (let ((nums (map (lambda (expr)
                    (set! exp expr)
                    (set! pc value-of-simple-exp)
                    (expval->num  (trampoline!))) exps)))
            (set! pc #f)
            (set! val
    ...
    (lambda ()
      (cases tfexp exp
        ...
        (cps-let-exp (var rhs body)
          (let ((val2 (begin (set! exp rhs)
                            (set! pc value-of-simple-exp)
                            (trampoline!))))
            (set! exp body)
            (set! env (extend-env* (list var) (list val2) env))
            (set! pc value-of/k)
            (trampoline!)))
        ...
        (cps-if-exp (simple1 body1 body2)
          ...
          (if (expval->bool (value-of-simple-exp))
            (begin (set! exp body1) (set! pc value-of/k))
            (begin (set! exp body2) (set! pc value-of/k))))
        (cps-call-exp (rator rands)
          (let ((proc-vals
                 (expval->proc
                  ((lambda ()
                     (set! exp rator) (set! pc value-of-simple-exp) (trampoline!)))))
                (rand-vals
                 (map (lambda (simple)
                        (set! exp simple)
                        (set! pc value-of-simple-exp)
                        (trampoline!)) rands)))
            (set! proc11 proc-vals)
            (set! args rand-vals)
