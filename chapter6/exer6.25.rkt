...
(define the-lexical-spec
 ...
 (define cps-out-grammar
    '((cps-out-program (tfexp) cps-a-program)
      ...
      (tfexp ("let" (arbno identifier "=" simple-expression) "in" tfexp)
             cps-let-exp)
      (tfexp
  ...
  (lambda (expression)
    (cases tfexp expression
      ...
      (cps-let-exp (vars exps body)
        (string-join
         (list "let"
          (string-join (map (lambda (var expr) (string-join (list var "=" expr)))
               (symbol->string vars) (format-simple-exp exps))) "in"
          (format-string body))))
